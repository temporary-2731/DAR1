...
class WeeklyEngineCheckActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            ...
            val engine = WeeklyAnalysisEngine(db)
            engine.debugEnabled = true

            val results = engine.computeAll(dslaId, generalActionId)

            logView.text = engine.debugLog.toString().ifEmpty { getString(R.string.check_engine_no_forms) }

            val builder = StringBuilder()
            builder.appendLine("=== WEEKLY RESULTS (${results.size} groups) ===\n")
